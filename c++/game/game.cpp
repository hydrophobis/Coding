...
int fight(Player& player) {
    ...
    int baseDamage = player.damage + player.sword.damage; // Include sword damage in base damage
    ...
    while (!enemies.empty()) {
        ...
        for (int i = 0; i < enemies.size(); ++i) {
            ...
            if (enemies[i].health > 0) {
                ...
                cout << "The " << enemies[i].name << " attacks you for " << RED << max(0, (enemyDamage - player.defense)) << RESET << " damage!\n\n\n";
...
void outputInv(const vector<int>& inventory, const vector<Item>& items) {
